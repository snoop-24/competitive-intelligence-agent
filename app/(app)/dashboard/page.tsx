    ...
    <div className="max-w-4xl mx-auto px-8 py-10 space-y-8">
      ...
      {competitors.length === 0 ? (
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-12 text-center">
          ...
          <h3 className="text-base font-semibold text-white mb-1">No competitors tracked yet</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
              <h2 className="font-semibold text-white mb-1">Generate Briefing</h2>
            ...
            {latestBriefing && (
              <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="font-semibold text-white">Latest Briefing</h2>
                  <Link href={`/briefings/${latestBriefing.id}`} className="text-sm text-violet-400 hover:underline">
                    View full →
                  </Link>
                </div>
                ...
                <p className="text-sm text-slate-300 leading-relaxed">{latestBriefing.executive_summary}</p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-slate-900 rounded-xl border border-slate-800 p-5">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Tracked</h2>
                <Link href="/competitors" className="text-xs text-violet-400 hover:underline">Manage</Link>
              </div>
              <ul className="space-y-3">
                {competitors.map(c => (
                  <li key={c.id} className="flex items-center gap-3">
                    <div className="w-7 h-7 rounded-lg bg-violet-500/20 flex items-center justify-center text-violet-400 font-bold text-xs flex-shrink-0">
                      {c.name[0].toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-white truncate">{c.name}</p>
                      <p className="text-xs text-slate-600 truncate">{c.website_url}</p>
                    </div>
                  </li>
                ))}
              </ul>
